...
mod inherit_tags_for_file_tests {
    use crate::test::{cleanup, create_file_db_entry, create_folder_db_entry, create_tag_folder, init_db_folder};
    use crate::repository::open_connection;
    use crate::tags::repository as tag_repository;

    #[test]
    fn inherits_tag_from_direct_parent() {
        init_db_folder();
        create_folder_db_entry("top", None); // folder id 1
        create_tag_folder("tag1", 1); // tag on folder 1
        create_file_db_entry("test_file", Some(1)); // file id 1 in folder 1
        
        // Check if file inherited the tag
        let con = open_connection();
        let tags = tag_repository::get_tags_on_file(1, &con).unwrap();
        con.close().unwrap();
        
        assert_eq!(1, tags.len(), "File should have 1 inherited tag");
    ...
    #[test]
    fn inherits_tag_from_ancestor() {
        init_db_folder();
        create_folder_db_entry("top", None); // folder id 1
        create_folder_db_entry("middle", Some(1)); // folder id 2
        create_folder_db_entry("bottom", Some(2)); // folder id 3
        create_tag_folder("tag1", 1); // tag on top folder
        create_file_db_entry("test_file", Some(3)); // file in bottom folder
        
        // Check if file inherited the tag from grandparent
        let con = open_connection();
        let tags = tag_repository::get_tags_on_file(1, &con).unwrap();
        con.close().unwrap();
