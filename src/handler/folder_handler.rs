...
pub fn update_folder(
    ...
    match folder_service::update_folder(&folder) {
        ...
        Err(UpdateFolderError::ParentNotFound) => UpdateFolderResponse::ParentNotFound(
            BasicMessage::new("The parent folder with the passed id could not be found."),
        ),
        Err(UpdateFolderError::AlreadyExists) => {
            UpdateFolderResponse::FolderAlreadyExists(BasicMessage::new(
                "Cannot update folder, because another one with the new path already exists.",
            ))
        }
        Err(UpdateFolderError::FileAlreadyExists) => UpdateFolderResponse::FolderAlreadyExists(
            BasicMessage::new("A file with that name already exists."),
        ),
        Err(UpdateFolderError::DbFailure) => {
            UpdateFolderResponse::FolderDbError(BasicMessage::new(
                "Could not update the folder in the database. Please check the server logs for more details.",
            ))
        }
        Err(UpdateFolderError::FileSystemFailure) => UpdateFolderResponse::FileSystemError(
            BasicMessage::new("Could not move the folder! Please see server logs for details."),
        ),
        Err(UpdateFolderError::NotAllowed) => UpdateFolderResponse::FolderAlreadyExists(
            BasicMessage::new("Cannot move parent folder into its own child."),
        ),
        Err(UpdateFolderError::TagError) => UpdateFolderResponse::TagError(BasicMessage::new(
            "Failed to update tags. Check server logs for details.",
        )),
    }
}
...
pub fn delete_folder(
    ...
    match folder_service::delete_folder(id) {
        ...
        Err(DeleteFolderError::FolderNotFound) => DeleteFolderResponse::FolderNotFound(
            BasicMessage::new("The folder with the request id does not exist."),
        ),
        Err(DeleteFolderError::DbFailure) => {
            DeleteFolderResponse::FolderDbError(BasicMessage::new(
                "Failed to remove folder reference from the database. Check server logs for details.",
            ))
        }
        Err(DeleteFolderError::FileSystemError) => {
            DeleteFolderResponse::FileSystemError(BasicMessage::new(
                "Failed to remove folder from the file system. Check server logs for details.",
            ))
        }
    }
}
